...
def tree_minimum(z):
    while z!=None and z.left!=None:
        z=z.left
    return z

def tree_maximum(z):
    while z!=None and z.right!=None:
        z=z.right
    return z
...
def find_nearest_unhaunted(floor_tree,floor_num):
    ...
    x=floor_tree.root
    closest=x
    z=tree_search(x,closest,floor_num)
    return z
        
        
    
def tree_search(x,closest,floor_num):
    if x==None:
        return closest
    elif   floor_num==x.floor_num:
        return x
    else:
        if floor_num>x.floor_num:
            min=abs(tree_minimum(x).floor_num-floor_num)
        else:
            min=abs(tree_maximum(x).floor_num-floor_num)
        if floor_num<x.floor_num:
            if x.left!=None and abs(floor_num-x.left.floor_num)<min:
                min=abs(floor_num-x.left.floor_num)
                closest=x.left
            return tree_search(x.left,closest,floor_num)
        if floor_num>x.floor_num:
            if x.right!=None and abs(x.right.floor_num-floor_num)<min:
                min=abs(x.right.floor_num-floor_num)
                closest=x.right
            return tree_search(x.right,closest,floor_num)
