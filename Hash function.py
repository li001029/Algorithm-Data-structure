...
def database_insert(database,robot):
    ...
    i=hash_function(robot.shape)
    database.shape_table[i].append(robot)
    k=hash_function(robot.color)
    database.color_table[k].append(robot)
...
def database_search(database,field,value):
    ...
    c=[]
    if field=="name":
        for k in range(0,len(database.name_table[i])):
            if database.name_table[i][k].name==value:       
                c.append(database.name_table[i][k])
    if field=="shape":
        for k in range(0,len(database.shape_table[i])):
            if database.shape_table[i][k].shape==value:       
                c.append(database.shape_table[i][k])
    if field=="color":
       for k in range(0,len(database.color_table[i])):
            if database.color_table[i][k].color==value:       
                c.append(database.color_table[i][k])
    return c
#Takes Database object database, string field (must be "name","shape","color")
#and string value, representing what value for the given field to search for
...
def database_delete(database,field,value):
    pass
    c=database_search(database,field,value)
    for k in range(0,len(c)):
        i=hash_function(c[k].name)
        z=len(database.name_table[i])-1
        while z>=0:
            if database.name_table[i][z]==c[k]:
                del database.name_table[i][z]
            z-=1
        i=hash_function(c[k].shape)
        z=len(database.shape_table[i])-1
        while z>=0:
            if database.shape_table[i][z]==c[k]:
                del database.shape_table[i][z]
            z-=1
        i=hash_function(c[k].color)
        z=len(database.color_table[i])-1
        while z>=0:
            if database.color_table[i][z]==c[k]:
                del database.color_table[i][z]
            z-=1
